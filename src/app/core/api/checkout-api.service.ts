...
import { OrderDetail, OrderRequest, OrderStatus, OrderSummary, Quote, Receipt, SeasonTicket, VerificationMethod } from '../shop/checkout.models';
...
export class CheckoutApiService {
  ...
  validateTicket(cardNumber: string, verificationValue: string) { return this.http.post<{ valid: boolean; discountPercent: number; seasonTicketToken: string }>(`${this.base}/shop/season-ticket/validate`, { cardNumber, verificationValue }).pipe(timeout(20000)); }
  ...
  saveTicket(data: { cardNumber: string; seasonKey: string; active: boolean; validFrom: string | null; validUntil: string | null; verificationMethod: VerificationMethod | null; verificationValue?: string }, id?: string) { return id ? this.http.patch<SeasonTicket>(`${this.base}/admin/shop/season-tickets/${id}`, data) : this.http.post<SeasonTicket>(`${this.base}/admin/shop/season-tickets`, data); }
}
